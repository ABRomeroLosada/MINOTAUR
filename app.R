...
{
  sd.zt <- paste("sd",paste0("zt",sprintf(fmt = "%02d",seq(from=0,to=20,by=4))),sep="_")
  current.gene.expression.sd.dd <- gene.expression[gene.id,c(paste(sd.zt,1,sep="_"),
                                                             paste(sd.zt,2,sep="_"),
                                                             paste(sd.zt,3,sep="_"),
                                                             paste(sd.zt,6,sep="_"),
                                                             paste(sd.zt,7,sep="_"))]
  
  min.expression <- min(current.gene.expression.sd.dd)
  max.expression <- max(current.gene.expression.sd.dd)
  range.expression <- max.expression - min.expression
  
  expression.step <- floor(range.expression / 5)
  
  plot(as.numeric(current.gene.expression.sd.dd),type="o",lwd=3,col="red",axes=F,xlab="",ylab="FPKM",
       ylim=c(min.expression-expression.step,max.expression),
       cex.lab=1.3,main=gene.id,cex.main=2)
  axis(side=2,lwd=3)
  axis(side = 1,pos = min.expression - 1.1*expression.step, at = seq(from=1,to=30),
...
#gene.id<-selected.gene
#gene.expression <- total.gene.expression
plot.ld.sd <- function(gene.id, gene.expression)
{
  ld.zt <- paste("ld",paste0("zt",sprintf(fmt = "%02d",seq(from=0,to=20,by=4))),sep="_")
  current.gene.expression.ld <- gene.expression[gene.id,c(paste(ld.zt,1,sep="_"),
                                                             paste(ld.zt,2,sep="_"),
...
plot.ld.sd.ll <- function(gene.id, gene.expression)
{
  ...
  min.expression <- min(current.gene.expression.sd.ll, current.gene.expression.ld.ll)
  max.expression <- max(current.gene.expression.sd.ll, current.gene.expression.ld.ll)
  range.expression <- max.expression - min.expression
...
plot.ld.sd.dd <- function(gene.id, gene.expression)
{
  ...
  min.expression <- min(current.gene.expression.sd.dd, current.gene.expression.ld.dd)
  max.expression <- max(current.gene.expression.sd.dd, current.gene.expression.ld.dd)
  range.expression <- max.expression - min.expression
    ...
    fluidRow(
        ...
        column(
            ...
            conditionalPanel(condition = "input.navigation_bar == 'github'",
                             tags$div(align = "justify", tags$b("MINOTAUR,"), "is entirely developed using 
  ...
  if(input$omics == "rna")
  {
   ...
   #####Circacompare analysis
    ...
    output$circadian.plot <- renderPlot(
      ...
      res       = 120,
      expr = {
        plot.ld.sd.dd(gene.id=selected.gene, 
                   gene.expression=total.gene.expression)
      })
    ...
    new.time.points.order.sd <- c(paste("sd_", "zt00_", seq(from=1, to=3), sep=""),
                                  ...
                                  paste("sd_", "zt16_", seq(from=1, to=3), sep=""),
                                  paste("sd_", "zt20_", seq(from=1, to=3), sep=""))
    gene.expression.rain.sd <- gene.expression.SD.DD[new.time.points.order.sd]
    head(gene.expression.rain.sd)
    ...
    new.time.points.order.ld <- c(paste("ld_", "zt00_", seq(from=1, to=3), sep=""),
                                  ...
                                  paste("ld_", "zt16_", seq(from=1, to=3), sep=""),
                                  paste("ld_", "zt20_", seq(from=1, to=3), sep=""))
    gene.expression.rain.ld <- gene.expression.LD.DD[new.time.points.order.ld]
    head(gene.expression.rain.ld)
    ...
    new.rain.order <-c(paste( "zt0_", seq(from=1, to=3), sep=""),
                       ...
                       paste( "zt16_", seq(from=1, to=3), sep=""),
                       paste( "zt20_", seq(from=1, to=3), sep=""))
    names(gene.expression.rain.sd) <- new.rain.order
    names(gene.expression.rain.ld) <- new.rain.order
    ...
    new.time.points.order.ld.dd <- c(paste("ld_", "zt00_", seq(from=2, to=3), sep=""),
                               paste("ld_", "zt00_", seq(from=6, to=7), sep=""),
                               paste("ld_", "zt04_", seq(from=2, to=3), sep=""),
                               paste("ld_", "zt04_", seq(from=6, to=7), sep=""),
                               paste("ld_", "zt08_", seq(from=2, to=3), sep=""),
    ...
    new.time.points.order.sd.dd <- c(paste("sd_", "zt00_", seq(from=2, to=3), sep=""),
                                     ...
                                     paste("sd_", "zt20_", seq(from=2, to=3), sep=""),
                                     paste("sd_", "zt20_", seq(from=6, to=7), sep=""))
    gene.expression.rain.sd.dd <- gene.expression.SD.DD[new.time.points.order.sd.dd]
    
    
    new.rain.order.dd <-c(paste( "zt0_", seq(from=2, to=5), sep=""),
                       paste( "zt4_", seq(from=2, to=5), sep=""),
                       paste( "zt8_", seq(from=2, to=5), sep=""),
                       paste( "zt12_", seq(from=2, to=5), sep=""),
                       paste( "zt16_", seq(from=2, to=5), sep=""),
                       paste( "zt20_", seq(from=2, to=5), sep=""))
    names(gene.expression.rain.sd.dd) <- new.rain.order.dd
    names(gene.expression.rain.ld.dd)<- new.rain.order.dd
    
    library(rain)
    rain24.ld.dd <- rain(as.numeric(gene.expression.rain.ld.dd), deltat=4, period=24, verbose=T, nr.series=3)
    rain12.ld.dd <- rain(as.numeric(gene.expression.rain.ld.dd), deltat=4, period=12, verbose=T, nr.series=3)
    rain24.sd.dd<- rain(as.numeric(gene.expression.rain.sd.dd), deltat=4, period=24, verbose=T, nr.series=3)
    rain12.sd.dd<- rain(as.numeric(gene.expression.rain.sd.dd), deltat=4, period=12, verbose=T, nr.series=3)
